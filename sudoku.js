...
function solveGame(numStr){
	...
	console.log("gp:", gP, "rs:", rS, "cs:", cS, "qs:", qS, "pe:", pE);
}
				...
				if(p2 === p[1]){
					...
					gameArr.forEach(function(game){
						if(game.val === 0 && game.row === x && game.col === p[0] + 1){
							...
							rS++;
							//console.log("rs", p2, game.row, game.col);
						}
					});
				}
			});
				...
				if(p2 === p[1]){
					...
					gameArr.forEach(function(game){
						if(game.val === 0 && game.col === x && game.row === p[0] + 1){
							...
							delete game.possible;
							cS++;
							//console.log("cs", p2, game.row, game.col);
						}
					});
				}
			});
		...
		pA2.forEach(function(p2){
			pA.forEach(function(p){
				if(p2 === p[1]){
					//val of square = p2
					gameArr.forEach(function(game){
						if(game.val === 0 && game.quad === x && game.row === quadIndex[x - 1][p[0]][0] && game.col === quadIndex[x - 1][p[0]][1]){
							game.val = p2;
							delete game.possible;
							qS++;
							//console.log("qs", p2, game.row, game.col);
						}
					});
				}
			});
	...
	for(x = 1; x <= 9; x++){
		...
		quadReq.forEach(function(req){
			...
			if(req.rows.length === 1){
				gameArr.forEach(function(square){
					if(square.row === req.rows[0] && square.quad != x && square.possible){
			...
			if(req.columns.length === 1){
				gameArr.forEach(function(square){
					if(square.col === req.columns[0] && square.quad != x && square.possible){
						var index = square.possible.indexOf(req.val)
						if(index >= 0){ 
							square.possible.splice(index, 1); 
							pE++;
...
function boardOut(){
	var str = "";
	gameArr.forEach(function(square, index){
		str += (gameArr[index].row === 4 || gameArr[index].row === 7) && gameArr[index].col === 1 ? "-".repeat(21) + "\n" : "";
...
function strToGame(str){
	valArr = String(str).split("").map(n => Number(n));
